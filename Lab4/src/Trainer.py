...
class VAE_Model(nn.Module):
    ...
    def training_stage(self):
        # with open('loss_curve.csv', 'w', newline='') as csvfile:
    ...
    def eval(self):
        # if args.test:
        #     with open('PSNR_diagram.csv', 'w', newline='') as csvfile:
        #         writer = csv.writer(csvfile)
        ...
        for (img, label) in (pbar := tqdm(val_loader, ncols=120)):
            ...
            # with open('loss_curve.csv', 'a', newline='') as csvfile:
            #     writer = csv.writer(csvfile)
            #     writer.writerow([loss])
    ...
    def val_one_step(self, img, label):
        ...
        with torch.no_grad():
            ...
            prev_img=img[0]
            for i in range(629):
                z=torch.randn_like(torch.empty(1, args.N_dim, 32, 64)).to(self.args.device)

                frame=self.frame_transformation(prev_img)
                pose=self.label_transformation(label[i+1])
                decoder=self.Decoder_Fusion(frame, pose, z)
                output_img=self.Generator(decoder)
                prev_img=output_img

                # if args.test:
                #     P=Generate_PSNR(img[i+1], output_img)
                #     with open('PSNR_diagram.csv', 'a', newline='') as csvfile:
                #         writer = csv.writer(csvfile)
                #         writer.writerow([P])
